...
describe('RC-07 Task 1: Static Architecture & Boundary Verification', () => {
  ...
  test('Zero unowned RC-07 composite tools exposed in TOOL_SCHEMAS or production server list (Task 7: count 25)', async () => {
    ...
    assert.ok(TOOL_SCHEMAS.arc_verify !== undefined, 'arc_verify must be in TOOL_SCHEMAS');
    ...
    assert.ok(
      TOOL_SCHEMAS.arc_stage_evidence !== undefined,
      'arc_stage_evidence must be in TOOL_SCHEMAS',
    );
    assert.ok(
      toolNames.includes('arc_stage_evidence'),
      'arc_stage_evidence must be in ALL_TOOL_DEFINITIONS',
    );

    // Exact count in Task 7 is 25 (18 base + 2 Task 2 tools + 1 Task 3 tool + 1 Task 4 tool + 1 Task 5 tool + 1 Task 6 tool + 1 Task 7 tool)
...
describe('RC-07 Task 1 Forward Security Corrections (Proofs A through O)', () => {
  ...
  test('Proof G: Production createArcMcpServer still constructs and holds exactly the intended execution authority internally and uses createProductionDeterministicRegistry()', async () => {
    ...
    assert.equal(reg.listEntryIds().length, 5); // Task 5 production registry has 5 entries
