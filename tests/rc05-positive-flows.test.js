...
async function startRemoteServer({
  ...
  const { storePath, records, store } = createTrustStoreFile(tag, pinCount);
  ...
  await server.start();
  return { server, port, storePath, records, store, audit, approvals, registry };
}
...
describe('CesSpace ARC — RC-05 Eleven Positive Acceptance Flows', () => {
  ...
  test('Flow 4: First-device local enrollment initiation & remote completion', async () => {
    ...
    try {
      // 1. Operator initiates pending enrollment
      ...
      // Verify audit has DEVICE_ENROLLED
      const records = server.auditLogger.getRecords();
      assert.ok(records.some((r) => r.gateway?.eventType === 'DEVICE_ENROLLED'));
    } finally {
      await server.stop();
    }
  });
  ...
  test('Flow 9: Authenticated local operator approval + same-session redemption', async () => {
    ...
    try {
      ...
      // 2. Operator approves on the local channel
      const grant = approvals.approve(requestId);
      assert.ok(grant.token);

      // 3. Client redeems from same session
  ...
  test('Flow 11: Durable device revocation & immediate session teardown', async () => {
    ...
    try {
      ...
      // 2. Operator revokes device via device trust store authority
      const deviceId = records[0].deviceId;
      const authority = server.remoteGateway.bootstrap.authoritativeTrustStore;
      authority.revokeDevice(deviceId);

      // Tear down device sessions on surface
