...
5. **Jailed Working Directory & Trusted Resolution:** Working directories must resolve strictly within the canonical authorized workspace root. Executables are resolved strictly from fixed trusted system directories (`/usr/bin`, `/bin`, `/usr/local/bin`). Resolution from user-controlled paths or `node_modules/.bin` is strictly forbidden.
...
| **`run_command`**       | Execute an approved dev tool.            | `executable`, `args` (<= 100), `cwd`, `timeoutMs` (100..300000), `env`, `runInBackground`. | Allowlist & profile check, `shell: false`, bounded output (512 KiB), secret scrubbing.     |
| **`process_status`**    | Query process execution state.           | `processId` (`arc-proc-*`).                                                                | Opaque ID validation, 3-part identity check (`clientId`, `sessionId`, `workspaceId`).      |
| **`process_output`**    | Read buffered process output.            | `processId`, `offset`, `maxBytes` (<= 128 KiB).                                            | Bounded chunk read, multi-byte UTF-8 safe, secret scrubbing.                               |
| **`terminate_process`** | Terminate an ARC-managed process.        | `processId`, `signal` (`SIGTERM` \| `SIGKILL`).                                            | Process group kill (`-pid`), SIGTERM with SIGKILL escalation, 3-part ownership validation. |

---

## 3. Acceptance Verification Summary

- **Total Test Cases:** 119/119 passing (54 RC-00/RC-01 + 65 RC-02 tests including 24 independent review regressions).
- **RC-02 Negative Controls:** 57 mandatory negative controls verified (denied commands, code execution bypasses, argument injection, shell metacharacters, env allowlist, strict schema, process ownership isolation, cwd containment, path traversal).
