...
- **Enforcement:** The Policy Engine kernel fallback logic evaluates strictly to `PolicyDecision.DENY`.
...
- **Enforcement:** Starting from RC-01's Minimal Security Kernel, subsystem drivers (Filesystem, Git, Terminal, Processes) are isolated from transport layers and can only be invoked through the Policy Engine execution pipeline.
...
- **Enforcement:** Canonical path validation (`realpath`), descriptor-relative traversal (`openat2 RESOLVE_BENEATH`), and prefix checks assert target paths reside within the authorized directory boundary. Symlinks escaping the root are rejected.
...
- **Statement:** Every privileged operation, policy evaluation, approval decision, and execution outcome must produce a structured audit record.
- **Enforcement:** Starting from RC-01, a structured audit sink records every request before and after execution with data minimization and redaction.
...
- **Enforcement:** Automated pre-commit checks and CI secret scanners (Gitleaks and repository policy scripts).
...
- **Verification:** Git history scanning in CI via Gitleaks.
