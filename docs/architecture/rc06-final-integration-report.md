...
- **Path & Env Minimization:** For path minimization, `target.workspacePath` is omitted/empty, while the trusted `workspaceRootHash` may remain. Absolute host filesystem paths in persisted free text become `[REDACTED_PATH]`. Environment variables and process arguments are minimized to prevent secret leakage.
...
| `RC06-NEG-100..104` | Central secrecy hardening, session tokens, authorization headers, PEM keys/certs, env minimization, workspace path minimization (`target.workspacePath` omitted/empty, trusted `workspaceRootHash` retained, absolute host paths redacted to `[REDACTED_PATH]`) | `tests/rc06-negative-controls.test.js`  | PASS   |
...
6. **RC06-FLOW-06: Operator Approval Lifecycle Persistence** — Approval request creation, operator approval via authenticated local Admin IPC (Unix socket, challenge-response, Ed25519 operator signing), and tool redemption with `_arcApproval` are logged with complete cryptographic hashes and contiguous sequence linkage.
7. **RC06-FLOW-07: Gateway Admission & Lifecycle Auditing** — Real gateway startup, client admission via mTLS, real gateway rate-limit enforcement, and clean shutdown events are recorded into the durable audit chain, with production-generated `RATE_LIMITED` records verified on disk before any supplemental vocabulary emission.
