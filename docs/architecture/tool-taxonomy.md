...
1. **Read-Only Inspection Tools (Tier 1 — RC-01 Target):** Safely observe filesystem state, Git status, and system health within authorized workspace roots, gated by the Minimal Security Kernel.
...
Read-only tools provide visibility into development environments without altering state. In RC-01, they are admitted exclusively through the **Minimal Security Kernel**, which enforces default-deny admission, canonical workspace root binding, and audit logging.
